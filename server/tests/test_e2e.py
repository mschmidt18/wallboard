def test_full_workflow(client):
    ...
    # Create layout
    layout = client.post("/api/layouts", json={"name": "E2E Test", "columns": 12, "row_height": 80}).json()
    layout_id = layout["id"]
    ...
    client.put(f"/api/layouts/{layout_id}/widgets/positions", json=[
        {"id": w["id"], "position_x": i * 4, "position_y": 0, "width": 4, "height": 2}
        for i, w in enumerate(widgets)
    ])

    # Verify updated positions
    updated = client.get("/api/display").json()
